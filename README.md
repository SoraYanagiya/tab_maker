...
五線譜入力画面には「操作」「音符入力」の2モードがある（ツールバー左端で切り替え、Escで操作モードへ）。**音符入力モード**（既定）では五線のクリックで音符を追加できる。**操作モード**ではクリックでは何も追加せず、既存の音符の選択・ドラッグだけを行う（誤って音符を増やさずに見たり動かしたりしたいときに使う）。

| 操作 | 内容 |
| --- | --- |
| 五線をクリック | その音高で音符を追加（音符入力モードのみ） |
