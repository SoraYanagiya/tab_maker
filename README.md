...
## MusicXML / MIDI インポートについて

複数のパート（トラック）を含むファイルを読み込むと、パート選択のダイアログが出る（パートが1つしかない場合はそのまま読み込む）。パート名はトラック名→楽器名→「トラック N」の順で決まる。打楽器トラック（高さを持たない音）は自動的にスキップされ、クラッシュせず警告として表示される。既定で選ばれるのは、実際に音符を含む最初のパート（打楽器トラックが先頭にあっても飛ばされる）。
